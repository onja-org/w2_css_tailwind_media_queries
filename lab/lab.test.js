...
describe('Tailwind Media Queries Lab', function() {
    ...
    describe('Challenge 1: The Responsive Card', function() {
        ...
        beforeEach(function() {
            ...
            cardContent = cardContainer?.querySelector('div');
        ...
        it('should have a responsive flex layout for tablet and desktop', function() {
            ...
            expect(cardContainer.className).to.include('md:flex',
        ...
        it('should have responsive image sizing (hint: use md:w-32 md:h-32)', function() {
            ...
            expect(imageClasses).to.match(/md:w-\d+/,
                'Image should have responsive width class like md:w-32');
            expect(imageClasses).to.match(/md:h-\d+/,
                'Image should have responsive height class like md:h-32');
        });
        ...
        it('should have proper content spacing for tablet layout', function() {
            ...
            expect(contentClasses).to.include('md:mt-0',
                'Content should have md:mt-0 to remove top margin on tablet');
            expect(contentClasses).to.match(/md:ml-\d+/,
                'Content should have left margin on tablet like md:ml-4');
        });

        it('should have responsive typography sizing', function() {
            expect(cardTitle).to.exist;
            const titleClasses = cardTitle.className;
            expect(titleClasses).to.match(/(md:text-|lg:text-)/,
        ...
        it('should have hover effects for desktop (hint: use lg:hover:shadow-lg)', function() {
            ...
            expect(containerClasses).to.match(/lg:hover:/,
        ...
        it('should have responsive button styling', function() {
            ...
            expect(buttonClasses).to.include('md:w-auto',
        ...
        it('should maintain mobile-first approach (base styles for mobile)', function() {
            expect(cardContainer).to.exist;
            expect(cardContainer.className).to.include('rounded-lg',
                'Card should have base mobile styles like rounded-lg');
            expect(cardImage.className).to.include('w-full',
                'Image should be full width on mobile');
            expect(cardButton.className).to.include('w-full',
    ...
    describe('Challenge 2: The Adaptive Navigation', function() {
        ...
        it('should hide hamburger menu on tablet and desktop', function() {
            ...
            expect(hamburgerButton.className).to.include('md:hidden',
                'Hamburger button should be hidden on tablet+ with md:hidden');
        });

        it('should have responsive header layout', function() {
            expect(navHeader).to.exist;
            const headerClasses = navHeader.className;
            expect(headerClasses).to.include('md:w-auto',
                'Header should have md:w-auto for proper tablet layout');
        });
        ...
        it('should transform menu to horizontal layout on tablet', function() {
            ...
            expect(menuClasses).to.include('md:flex',
                'Menu should have md:flex for horizontal layout on tablet');
            expect(menuClasses).to.include('md:items-center',
                'Menu should have md:items-center for proper alignment');
        });

        it('should have responsive menu spacing', function() {
            expect(navMenu).to.exist;
            const menuClasses = navMenu.className;
            expect(menuClasses).to.include('md:mt-0',
                'Menu should have md:mt-0 to remove top margin on tablet');
            expect(menuClasses).to.include('md:space-y-0',
                'Menu should have md:space-y-0 to remove vertical spacing on tablet');
            expect(menuClasses).to.match(/md:space-x-\d+/,
        ...
        it('should have responsive menu item layout', function() {
            ...
            navItems.forEach(item => {
                ...
                expect(link.className).to.include('block',
                    'Menu links should be block elements');
                expect(link.className).to.match(/py-\d+/,
                    'Menu links should have vertical padding');
            });
        });
        ...
        it('should show extra content only on desktop', function() {
            ...
            expect(extraClasses).to.include('hidden',
                'Extra content should be hidden on mobile');
            expect(extraClasses).to.include('lg:block',
                'Extra content should be visible on desktop with lg:block');
        });

        it('should maintain proper navigation structure', function() {
            expect(nav).to.exist;
            expect(nav.className).to.include('bg-gray-800',
                'Navigation should maintain base styling');
            expect(navMenu.tagName).to.equal('UL',
                'Navigation should use semantic HTML with ul element');
            expect(navItems.length).to.be.at.least(3,
                'Navigation should have at least 3 menu items');
        });
    });
    ...
    describe('Challenge 3: The Content Choreographer', function() {
        ...
        beforeEach(function() {
            ...
            mainContent = dashboard?.querySelector('.bg-blue-50');
            secondaryContent = dashboard?.querySelector('.bg-green-50');
            tertiaryContent = dashboard?.querySelector('.bg-purple-50');
            sidebar = dashboard?.querySelector('.bg-gray-50');
            statsContainer = mainContent?.querySelector('.mt-4');
        });

        it('should have responsive grid layout', function() {
            expect(dashboard).to.exist;
            const dashboardClasses = dashboard.className;
            expect(dashboardClasses).to.include('grid',
                'Dashboard should use CSS Grid');
            expect(dashboardClasses).to.include('grid-cols-1',
                'Dashboard should have 1 column on mobile');
            expect(dashboardClasses).to.include('md:grid-cols-2',
                'Dashboard should have 2 columns on tablet');
            expect(dashboardClasses).to.include('lg:grid-cols-3',
                'Dashboard should have 3 columns on desktop');
        });

        it('should have proper grid gap spacing', function() {
            expect(dashboard).to.exist;
            const dashboardClasses = dashboard.className;
            expect(dashboardClasses).to.match(/gap-\d+/,
                'Dashboard should have gap spacing between grid items');
        });
        ...
        it('should hide secondary content on mobile, show on tablet+', function() {
            ...
            expect(secondaryClasses).to.include('hidden',
                'Secondary content should be hidden on mobile');
            expect(secondaryClasses).to.include('md:block',
        ...
        it('should hide tertiary content on mobile and tablet, show on desktop', function() {
            ...
            expect(tertiaryClasses).to.include('hidden',
                'Tertiary content should be hidden on mobile');
            expect(tertiaryClasses).to.include('lg:block',
        ...
        it('should hide sidebar on mobile/tablet, show on desktop', function() {
            ...
            expect(sidebarClasses).to.include('hidden',
                'Sidebar should be hidden on mobile');
            expect(sidebarClasses).to.include('lg:block',
                'Sidebar should be visible on desktop with lg:block');
        });

        it('should have responsive stats layout', function() {
            expect(statsContainer).to.exist;
            const statsClasses = statsContainer.className;
            expect(statsClasses).to.include('md:flex',
                'Stats should have horizontal layout on tablet with md:flex');
            expect(statsClasses).to.match(/md:space-x-\d+/,
                'Stats should have horizontal spacing on tablet');
        });
        ...
        it('should maintain main content visibility across all breakpoints', function() {
            ...
            expect(mainClasses).to.not.include('hidden',
                'Main content should always be visible');
            expect(mainContent.textContent).to.include('Main Dashboard',
        ...
        it('should demonstrate progressive enhancement', function() {
            ...
            expect(allContent.length).to.be.at.least(4,
                'Dashboard should have multiple content sections');

            // Count hidden elements (should have some for progressive disclosure)
            const hiddenElements = dashboard.querySelectorAll('.hidden');
            expect(hiddenElements.length).to.be.at.least(2,
    ...
    describe('Breakpoint-Specific Tests', function() {
        ...
        it('should have medium breakpoint classes (md: 768px+)', function() {
            ...
            expect(html).to.match(/md:(flex|grid|block|hidden)/,
                'Should use md: for layout changes');
        });
        ...
        it('should have large breakpoint classes (lg: 1024px+)', function() {
            ...
            expect(html).to.match(/lg:(hover|block)/,
                'Should use lg: for desktop features like hover effects');
        });
    ...
    describe('Challenge Integration Tests', function() {
        it('should work together as a cohesive responsive system', function() {
            ...
            challenges.forEach((challenge, index) => {
                expect(challenge.id).to.equal(`challenge-${index + 1}`,
                    'Challenges should have proper IDs');
            });
        });
        ...
        it('should have consistent responsive patterns across challenges', function() {
            ...
            expect(flexElements.length).to.be.at.least(2,
                'Should use flex layout consistently');
            expect(hiddenElements.length).to.be.at.least(3,
                'Should use progressive disclosure consistently');
            expect(gridElements.length).to.be.at.least(1,
                'Should use grid layout where appropriate');
        });
        ...
        it('should provide educational value through examples', function() {
            ...
            expect(hints.length).to.be.at.least(3,
                'Should provide helpful hints for each challenge');

            // Check for code examples in hints
            hints.forEach(hint => {
                expect(hint.textContent).to.include('md:',
                    'Hints should include concrete examples');
            });
        });
    });
});
